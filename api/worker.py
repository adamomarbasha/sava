...
Deployment: one worker process per machine is usually enough. Claiming is
atomic, so running several is safe — on Postgres via FOR UPDATE SKIP LOCKED,
on SQLite via serialised writes.
"""
...
def run_loop(worker_index: int, poll_seconds: float) -> None:
    ident = f"{WORKER_ID}#{worker_index}"
    idle = 0
    while not _stop.is_set():
        db = SessionLocal()
        try:
            job = claim_next(db, worker_id=ident)
            if job is None:
                idle += 1
...
def main() -> int:
    ...
    threads = [
        threading.Thread(target=run_loop, args=(i, args.poll), daemon=True,
                         name=f"sava-worker-{i}")
