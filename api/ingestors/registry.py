...
def add_bookmark(url: str, user_id: int, db: Session = None) -> Dict[str, Any]:
    ...
    try:
        ...
        if existing:
            ...
            return _create_new_bookmark(ingestor, url, user_id, db)
...
def _create_new_bookmark(ingestor: BaseIngestor, url: str, user_id: int, db: Session) -> Dict[str, Any]:
    try:
...
def _update_existing_bookmark(bookmark: Bookmark, ingestor: BaseIngestor, url: str, db: Session) -> Dict[str, Any]:
    try:
        raw_data = ingestor.extract_metadata(url)
        normalized = ingestor.normalize_metadata(raw_data)
