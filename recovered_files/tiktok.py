...
async def add_bookmark(url: str, user_id: int, db: Session = None, user_note: str = None) -> Dict[str, Any]:
    ...
    try:
        ...
        if existing:
            ...
            return await _create_new_bookmark(ingestor, url, user_id, db, user_note)
...
async def _create_new_bookmark(ingestor: BaseIngestor, url: str, user_id: int, db: Session, user_note: str = None) -> Dict[str, Any]:
    try:
        if ingestor.is_async():
        ...
        if ingestor.platform == "tiktok" and not title and user_note:
            title = user_note.strip()[:500] if user_note.strip() else None
