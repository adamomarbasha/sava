...
// Two intents, both of which save a **link**:
//
//   Save to Sava        — optional input; empty means "use the clipboard".
//                         This is what the Action Button runs.
//   Save Link to Sava   — takes the URLs from "Get What's On Screen".
//
...
// "Show When Run = Off".
//
// There is no screenshot action any more. See `CapturePipeline` for why: a
// screenshot cannot establish which post it shows, so it could only ever
// produce a confident guess at the wrong video.
...
private enum CaptureRunner {
    ...
    static func run(candidates: [String], inputTypes: [String]) async -> String {
        let started = Date()
        var trace = CaptureTrace()
        trace.hadShortcutInput = !candidates.isEmpty
        ...
        // Clipboard: the Action Button journey, and the only route that works
        // on Instagram.
        //
        ...
        // Copy Link is not a fallback there — it is the intended workflow, and
        // it is what the Action Button demo in onboarding teaches.
        //
...
struct SaveToSavaIntent: AppIntent {
    ...
    func perform() async throws -> some IntentResult & ProvidesDialog {
        ...
        let message = await CaptureRunner.run(
            candidates: candidates, inputTypes: types)
        return .result(dialog: IntentDialog(stringLiteral: message))
    }
